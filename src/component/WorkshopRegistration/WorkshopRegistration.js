...
class WorkshopRegistration extends Component {
    ...
    Schema = {
        title: Joi.string().required().label("Workshop Title").max(20),
        subject: Joi.string().required().label("Subject").min(1).max(20),
    ...
    registerWorkshop(event) {
        ...
        if (workshop.proposal === null) {
            ...
            formData.append('proposal', workshop.proposal);
    ...
    render() {
        ...
        const { loading, errors, workshop, buttonError } = this.state   //properties
                ...
                <Container className="workshopregistration-container">
                    <Form autoComplete="off" onSubmit={registerWorkshop}>
                        ...
                        <Form.Group className="paperregistration-form-group">
                            <Form.File name="proposal" label="Attach Proposal here" onChange={this.onFileChange} accept=".pdf" />
                            <Form.Text className="text-muted">
                                File format: PDF
                            </Form.Text>
                        </Form.Group>
                        <Button type="submit" variant="dark" disabled={this.validate()} className="workshop1-button">Submit</Button>
                        {buttonError && <p className="paperregistration-error">{buttonError}</p>}
                    </Form>
                </Container >
