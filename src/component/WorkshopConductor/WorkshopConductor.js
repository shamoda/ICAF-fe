...
import { Form, Container, Button, Modal, Spinner, Col, Row, Alert } from 'react-bootstrap';
...
class WorkshopRegistration extends Component {
    ...
    passwordChange = ({ target: input }) => {
        ...
        let workshop = { ...this.state.workshop }
        const errors = { ...this.state.errors }
        workshop[input.name] = input.value, this.setState({ workshop }, () => console.log(this.state));
        //verify passwords
    ...
    Schema = {
        ...
        qualification1: Joi.string().required().label("Qualifications 1"),
    ...
    registerWorkshop(event) {
        ...
        if (workshop.password !== workshop.passwordconfirmation) {
            ...
            formData.append('edu1', workshop.eduQualification1);
            formData.append('edu2', workshop.eduQualification2);
    ...
    render() {
        const { workshop, passwordMatch } = this.state
        const { loading, errors } = this.state   //properties
        const { handleChange, onFileChange, registerWorkshop, passwordChange } = this //methods
        return (
                ...
                <Container className="workshopregistration-container">
                    <Form autoComplete="off" onSubmit={registerWorkshop}>
                        <div className="workshopregistration-head">
                            <h4 className="workshopregistration-head-font">  <FontAwesomeIcon icon={faUserShield} />Personal Details</h4>
                        ...
                        <h4 className="workshopregistration-head-font" style={{ marginLeft: "400px", marginTop: "40px" }}>  <FontAwesomeIcon icon={faUserShield} />Create Account</h4>
                        <div style={{ marginLeft: "-400px", marginTop: "30px" }}>
                            ...
                            <div className="workshopregistration-head">
                                ...
                                <Form.Group as={Row} controlId="formPlaintextEmail">
                                    ...
                                    <Col sm="10">
                                        <Form.Control defaultValue="email@example.com" onChange={this.handleChange} readOnly value={workshop.email} />
                                ...
                                <Form.Group as={Row} controlId="password1">
                                    ...
                                    <Col sm="10">
                                        <Form.Control name="password" onChange={this.handleChange} value={workshop.password} type="password" placeholder="Password" />
                                        <Form.Text className="text-muted">
                                            Enter a password with capital,simple letter,numeric values and special characters..
                                        </Form.Text>
                                ...
                                <Form.Group as={Row} controlId="password2">
                                    ...
                                    <Col sm="10">
                                        <Form.Control name="passwordconfirmation" onChange={this.passwordChange} value={workshop.passwordconfirmation} type="password" placeholder="Password" />
                                        <Form.Text className="text-muted">
                                            Match the passwords......
                                        </Form.Text>
                                        {errors.passwordconfirmation && <Alert variant="danger" >
                                            <Alert.Heading style={{ fontSize: "15px" }}>
                                ...
                                <Button type="submit" variant="dark" className="workshopregistration-button" disabled={this.validate()}>Submit</Button>
                                {this.state.buttonError && <p className="paperregistration-error">{this.state.buttonError}</p>}
                            </div>
